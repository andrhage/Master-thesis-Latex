...
\begin{itemize}
    \item \textbf{Vanishing gradients at values close to 0 or 1}
    \item \textbf{The neurons can be saturated if the initial weights are too large}
    \item \textbf{The outputs are not zero-centered}
\end{itemize}
...
The ReLu function is described mathematically in two ways in this thesis, and both are illustrated in equation \ref{eq:reLu} and equation \ref{eq:reLu_2} for a better understanding of the function.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{bilder/relu_activation.png}
...
\todo{skriv om dropout, og l2 hvis det blir implementert}
